  ...
  return (
    ...
    >
      ...
      <Toolbar sx={{ justifyContent: "space-between", minHeight: 56 }}>
        {/* Left Side: Toggle, Title & Breadcrumbs */}
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          {/* Page Title as Toggle Button */}
          <Button
            onClick={onToggleSidebar}
            startIcon={isSidebarCollapsed ? <MenuIcon /> : <CollapseIcon />}
          ...
          >
            {pathArray.length > 0 
              ? (tb.has(pathArray[pathArray.length - 1]) 
                  ? tb(pathArray[pathArray.length - 1]) 
                  : pathArray[pathArray.length - 1]?.charAt(0).toUpperCase() + pathArray[pathArray.length - 1]?.slice(1))
              : tb("home")}
          </Button>
          ...
          {/* Breadcrumbs - Horizontally aligned with toggle */}
          <Breadcrumbs
            separator={<NavigateNextIcon sx={{ fontSize: "1rem", opacity: 0.5 }} />}
          ...
          >
            ...
            {pathArray.map((path, index) => {
              ...
              const label = tb.has(path) ? tb(path) : path.charAt(0).toUpperCase() + path.slice(1);

              return isLast ? (
                <Typography key={path} color="text.primary" sx={{ fontWeight: 700, fontSize: "0.8125rem" }}>
                  {label}
                </Typography>
              ) : (
                <Link key={path} component={NextLink} underline="hover" color="inherit" href={href} sx={{ fontWeight: 600, fontSize: "0.8125rem", color: "text.secondary" }}>
                  {label}
                </Link>
              );
            })}
