  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        {stats.map((stat) => (
          <Card
            ...
            sx={{
              ...
              "&:hover": { transform: "translateY(-1px)", boxShadow: "0 4px 8px rgba(0,0,0,0.05)" }
            }}
          >
            <CardContent sx={{ p: 1.2, "&:last-child": { pb: 1.2 } }} onClick={() => router.push(stat.link)}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr', lg: '1fr 1fr 1fr' }, gap: 1 }}>
        ...
        <Card sx={{ borderRadius: 3, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", border: "1px solid", borderColor: "divider" }}>
          <CardContent sx={{ p: 1.5 }}>
            ...
            <Box sx={{ width: '100%', height: 200 }}>
              <LineChart
                ...
                series={[
                  {
                    data: statsData?.attendanceTrend || [85, 92, 88, 95, 89, 94, 91],
                    area: true,
                    color: '#4f46e5',
            ...
            <Box sx={{ width: '100%', height: 200, display: 'flex', justifyContent: 'center' }}>
              <PieChart
                series={[
                  {
                    data: [
                      { id: 0, value: statsData?.attendanceToday || 0, label: t('status.on_time'), color: '#10b981' },
                      { id: 1, value: statsData?.lateToday || 0, label: t('status.late'), color: '#f59e0b' },
                      { id: 2, value: statsData?.leaveRequests || 0, label: t('status.leave'), color: '#ec4899' },
                    ],
                    innerRadius: 50,
                    outerRadius: 80,
                    paddingAngle: 5,
        ...
        <Card sx={{ borderRadius: 3, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", border: "1px solid", borderColor: "divider" }}>
          <CardContent sx={{ p: 1.5 }}>
            ...
            <Box sx={{ width: '100%', height: 200 }}>
              <BarChart
                xAxis={[{ 
                  data: Object.values(t.raw('months_short')).slice(0, new Date().getMonth() + 1), 
                  scaleType: 'band' 
                }]}
                series={[
                  { 
                    data: [4, 3, 5, 2, 6, 4, 3, 5, 2, 6, 4, 3].slice(0, new Date().getMonth() + 1), 
                    color: '#4f46e5', 
        ...
        <Card sx={{ borderRadius: 3, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", border: "1px solid", borderColor: "divider" }}>
          <CardContent sx={{ p: 1.5 }}>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
              ...
              <Button size="small" sx={{ fontSize: '0.75rem' }} onClick={() => router.push("/dashboard/attendance")}>
                {t('view_all')}
              </Button>
            </Box>
                ...
                <React.Fragment key={activity.id}>
                  <ListItem sx={{ px: 0, py: 1 }}>
                    ...
                    <ListItemText
                      ...
                      secondary={<Typography variant="caption" color="text.secondary">{activity.action}</Typography>}
                    />
                    <Box sx={{ textAlign: "right" }}>
                      ...
                      <Chip
                        ...
                        sx={{ height: 18, fontSize: "0.625rem", fontWeight: 700, borderRadius: 1 }}
                      />
                    </Box>
                  </ListItem>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", border: "1px solid", borderColor: "divider" }}>
            <CardContent sx={{ p: 1.5 }}>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <LinearProgress variant="determinate" value={statsData?.attendanceRate || 0} sx={{ height: 6, borderRadius: 3 }} />
              </Box>
              ...
              <Box sx={{ mb: 0 }}>
                ...
                <LinearProgress variant="determinate" value={statsData?.kpiCompletion || 0} color="secondary" sx={{ height: 6, borderRadius: 3 }} />
              </Box>
            </CardContent>
          </Card>

          <Paper sx={{ p: 1.5, bgcolor: "primary.main", color: "white", borderRadius: 3, boxShadow: "0 4px 12px rgba(79, 70, 229, 0.2)" }}>
            <Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}>
              <CheckCircleIcon fontSize="small" />
              <Box>
                <Typography variant="caption" sx={{ fontWeight: 700, display: 'block' }}>{t('system_notification.title')}</Typography>
                <Typography variant="caption" sx={{ opacity: 0.8, fontSize: '0.65rem' }}>{t('system_notification.desc')}</Typography>
              </Box>
            </Stack>
          </Paper>
        </Stack>
