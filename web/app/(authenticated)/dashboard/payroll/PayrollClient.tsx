...
export default function PayrollClient({ initialData, currentMonth, currentYear }: PayrollClientProps) {
  const t = useTranslations("Payroll");
  const [month, setMonth] = useState<number>(currentMonth);
  const [year, setYear] = useState<number>(currentYear);
  const [records, setRecords] = useState<PayrollRecord[]>(initialData || []);
  ...
  useEffect(() => {
    // If no initial data but we have month/year, try to fetch
